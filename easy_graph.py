...
def find_path(initial_edges, num_agents, agent_starts, agent_goals):
	...
	vert = [Function("v{}".format(i),IntSort(), BoolSort()) for i in range(num_agents)]
	...
	for agent in range(num_agents):
		...
		other_agents = [vert[a](x) for a in other_agents]
		if other_agents:
			s.add(ForAll( x,
						  Implies( vert[agent](x), 
								   Not( Or( *other_agents )))))
...
find_path([(0,1),(1,2),(2,3),(4,5),(5,6),(4,2)],2,[0,4],[[3,6]])
